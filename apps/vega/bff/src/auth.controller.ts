...
import { z } from 'zod';
import { AT_COOKIE, RT_COOKIE, type VegaBffEnv } from './config.js';
import { ENV } from './tokens.js';
import { bearerFrom, decodeJwt } from './upstream.js';
...
export class AuthController {
  ...
  private cookieOpts() {
    return { httpOnly: true, sameSite: 'lax' as const, secure: this.env.COOKIE_SECURE, path: '/' };
  }
  ...
  async login(@Body() raw: unknown, @Res({ passthrough: true }) res: Response) {
    ...
    res.cookie(AT_COOKIE, tokens.accessToken, this.cookieOpts());
    res.cookie(RT_COOKIE, tokens.refreshToken, this.cookieOpts());
